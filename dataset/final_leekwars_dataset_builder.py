#!/usr/bin/env python3
"""
Leek Wars Dataset Builder - Final Comprehensive Version
=====================================================

This script builds a comprehensive dataset from Leek Wars fight data:
1. Your own fight history (primary data source)
2. Option to collect from public fighters (limited by API access)

Features:
- Works with the authentication method that's confirmed functional
- Extracts all available fight data from your account
- Saves in both JSON and CSV formats
- Includes detailed metadata for analysis
...
def build_dataset(login: str, password: str, max_fights: int = 100) -> list:
    """
    Build dataset from Leek Wars fight history using authenticated user data
